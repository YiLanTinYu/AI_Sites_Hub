...
- 首页提供“30 秒选 AI”，先按用途和预算缩小范围。
- 场景推荐页面向普通用户解释“适合谁、怎么选、有哪些主流选择”。
- 订阅套餐页支持按场景、地区、预算、免费套餐和核实时间筛选排序。
- 推荐结果尽量避免同一家厂商刷屏，同一条件下优先展示不同厂商的主流套餐。
- 场景页可以作为 SEO 长尾入口，例如“日常问答 AI 怎么选”“写作办公 AI 怎么选”。
...
- 首页展示精选优惠，完整内容进入优惠专区
- 允许用户反馈优惠失效，但反馈需要审核
- 每日巡检发现页面变化、访问异常或临期活动时，可通过 Webhook 或微信提醒人工核查
...
场景推荐页使用 `/recommendations/[scenario]/` 路径，例如 `/recommendations/chat/` 和
`/recommendations/coding/`。这些页面必须保持通俗表达，优先帮助用户做选择，而不是堆砌模型参数。
...
- 固定人工核查来源：按 `人工核查.md` 每月至少核查 1 次
- 日常维护结果：填写 `日常维护核查表.md`，核实后再更新网站数据
- 即将到期优惠：每天更新状态
- 产品发现：每周运行 1 次
- 全站链接检查：每周运行 1 次

“新 AI 产品每周发现”会在每周一北京时间 10:23 自动运行，从 RSS、Hacker News、
GitHub 和 Hugging Face 汇总候选，过滤站内已有入口并按收录价值分级。任务会生成中文
精简清单（约 12 条最高分候选）、上传完整报告，并创建或更新 GitHub Issue 提醒人工审核；
候选不会自动发布。

“全站链接每周巡检”会在每周三北京时间 10:43 自动运行。它将 404、410 和明确服务器
错误列为确定需要处理，将 401、403、429、超时与临时网络错误保留为复查项；只有确定失效
时才创建或更新 GitHub Issue，复查恢复后自动关闭提醒，不会自动删除网站入口。
- 人工复核主流厂商价格：每月至少 1 次
...
如需把提醒发到微信，可以使用 Server 酱，并在同一位置添加：

```text
SERVERCHAN_SENDKEY=你的 Server 酱 SendKey
```

`DEAL_WEBHOOK_URL` 和 `SERVERCHAN_SENDKEY` 可以二选一，也可以同时配置。未配置时，
工作流会跳过通知并继续生成审核报告。

## 同类站对比

竞品与差距记录放在 `docs/竞品对比.md`。后续新增功能前，优先检查该文档：

- 同类站已有、但惠选AI缺失的高价值能力，优先评估是否补齐。
- 同类站偏专业、普通用户难理解的能力，需要转成通俗版再做。
- 不为了追求数量牺牲官方来源和人工核实。
